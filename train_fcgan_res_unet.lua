...
opt = {
   ...
   name = 'paris_fcgan_res_unet_wfeat50',   -- name of the experiment you are running
...
local netG = nil
-- input is (nc) x 128 x 128
local e1 = - nn.SpatialConvolution(nc, nef, 4, 4, 2, 2, 1, 1)
-- input is (nef) x 64 x 64
local e2 = e1 - nn.LeakyReLU(true) - nn.SpatialConvolution(nef, nef, 3, 3, 1, 1, 1, 1) - nn.SpatialBatchNormalization(nef)
-- input is (nef) x 64 x 64
local e3 = e2 - nn.LeakyReLU(true) - nn.SpatialConvolution(nef, nef * 2, 4, 4, 2, 2, 1, 1) - nn.SpatialBatchNormalization(nef * 2)
-- input is (nef * 2) x 32 x 32
local e4 = e3 - nn.LeakyReLU(true) - nn.SpatialConvolution(nef * 2, nef * 2, 3, 3, 1, 1, 1, 1) - nn.SpatialBatchNormalization(nef * 2)
-- input is (nef * 2) x 32 x 32
local e5 = e4 - nn.LeakyReLU(true) - nn.SpatialConvolution(nef * 2, nef * 4, 4, 4, 2, 2, 1, 1) - nn.SpatialBatchNormalization(nef * 4)
-- input is (nef * 4) x 16 x 16
local e6 = e5 - nn.LeakyReLU(true) - nn.SpatialConvolution(nef * 4, nef * 4, 3, 3, 1, 1, 1, 1) - nn.SpatialBatchNormalization(nef * 4)
-- input is (nef * 4) x 16 x 16
local e7 = e6 - basicblock(nef * 4, nef * 4, 1)
-- input is (nef * 4) x 16 x 16
local e8 = e7 - nn.LeakyReLU(true) - nn.SpatialConvolution(nef * 4, nef * 8, 3, 3, 1, 1, 1, 1) - nn.SpatialBatchNormalization(nef * 8)
-- input is (nef * 8) x 16 x 16
local e9 = e8 - basicblock(nef * 8, nef * 8, 1)
-- input is (nef * 8) x 16 x 16
local d1 = e9 - nn.ReLU(true) - nn.SpatialConvolution(nef * 8, ngf * 4, 3, 3, 1, 1, 1, 1) - nn.SpatialBatchNormalization(ngf * 4)
-- input is (ngf * 4) x 16 x 16
local d2 = d1 - nn.ReLU(true) - nn.SpatialFullConvolution(ngf * 4, ngf * 4, 4, 4, 2, 2, 1, 1) - nn.SpatialBatchNormalization(ngf * 4)
-- input is (ngf * 4) x 32 x 32
local d3_ = d2 - nn.ReLU(true) - nn.SpatialFullConvolution(ngf * 4, ngf * 2, 3, 3, 1, 1, 1, 1) - nn.SpatialBatchNormalization(ngf * 2)
-- input is (ngf * 2) x 32 x 32
local d3 = {d3_,e3} - nn.JoinTable(2)
local d4 = d3 - nn.ReLU(true) - nn.SpatialFullConvolution(ngf * 2 * 2, ngf * 2, 4, 4, 2, 2, 1, 1) - nn.SpatialBatchNormalization(ngf * 2)
-- input is (ngf * 2) x 64 x 64
local d5_ = d4 - nn.ReLU(true) - nn.SpatialFullConvolution(ngf * 2, ngf, 3, 3, 1, 1, 1, 1) - nn.SpatialBatchNormalization(ngf)
-- input is (ngf) x 64 x 64
local d5 = {d5_,e1} - nn.JoinTable(2)
local d6 = d5 - nn.ReLU(true) - nn.SpatialFullConvolution(ngf * 2, ngf / 2, 4, 4, 2, 2, 1, 1) - nn.SpatialBatchNormalization(ngf / 2)
-- input is (ngf / 2) x 128 x 128
local d7 = d6 - nn.ReLU(true) - nn.SpatialFullConvolution(ngf / 2, nc, 3, 3, 1, 1, 1, 1)
-- input is (nc) x 128 x 128
local o1 = d7 - nn.Tanh()
