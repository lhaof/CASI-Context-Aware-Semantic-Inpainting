   ...
   if n > 0 then
      ...
      self.threads = Threads(n,
                             ...
                             function(idx)
                                ...
                                if opt.quiet~=1 then print(string.format('Starting donkey with id: %d seed: %d', tid, seed)) end
                                assert(options, 'options not found')
                                assert(opt, 'opt not given')
                                if opt.quiet~=1 then print(opt) end
...
function data:getBatch()
   ...
   if opt.quiet~=1 then print(type(res)) end
   return res
end
